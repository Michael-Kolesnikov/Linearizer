...
START_TEST(test_function_call_without_arguments) {
    ...
    // Node* root =
    // create_function_declaration_node(return_type,function_declarator,create_empty_statement_node());
    FILE* output = tmpfile();
    ck_assert_ptr_nonnull(output);
    generate_code_from_ast(root, output);

    char* result = read_file_to_string(output);
    const char* expected = "function_call()";
...
START_TEST(test_function_call_with_one_argument) {
    ...
    char* result = read_file_to_string(output);
    const char* expected = "function_call(arg)";
    ck_assert_str_eq(result, expected);
...
START_TEST(test_function_call_with_onemore_arguments) {
    ...
    char* result = read_file_to_string(output);
    const char* expected = "function_call(arg1, arg2)";
    ck_assert_str_eq(result, expected);
